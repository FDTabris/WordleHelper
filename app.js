...
function focusNextCell(r, c) {
  if (c < COLS - 1) {
    const nextInRow = getCell(r, c + 1);
    if (nextInRow) nextInRow.focus();
    return;
  }

  if (r < ROWS - 1) {
    const nextRowFirst = getCell(r + 1, 0);
    if (nextRowFirst && !nextRowFirst.disabled) {
      nextRowFirst.focus();
    }
  }
}

function getPrevCell(r, c) {
  if (c > 0) return getCell(r, c - 1);
  if (r > 0) return getCell(r - 1, COLS - 1);
  return null;
}
